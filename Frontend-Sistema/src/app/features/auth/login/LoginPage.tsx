  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile.role === "admin") {
        navigate("/admin");
      } else if (profile.role === "enfermero") {
        navigate("/panel-enfermero");
      } else {
          ...
          <div className="grid min-h-[calc(100dvh-56px)] w-full overflow-hidden bg-white md:grid-cols-[1.05fr_0.95fr]">
            ...
            <div className="flex h-full items-center justify-center bg-white p-6 sm:p-10 lg:p-14">
              <div className="w-full max-w-md">
                ...
                {errorMsg && (
                  <div className="mt-4 p-3 bg-red-50 border border-red-200 text-red-700 rounded-xl text-xs font-semibold animate-in fade-in duration-300">
                    ⚠ {errorMsg}
                  </div>
                )}
                ...
                <form className="mt-8 space-y-5" onSubmit={handleLogin}>
                  ...
                  <button
                    ...
                    className="w-full rounded-xl bg-teal-500 py-3 font-semibold text-white transition hover:bg-teal-600 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isLoggingIn ? (
                      <>
